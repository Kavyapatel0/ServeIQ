...
export function LoginPage() {
  ...
  useEffect(() => {
    if (!googleClientId || showForgot) return;

    const handleCredentialResponse = async (response) => {
      const result = await googleLogin(response.credential);
      if (result.meta.requestStatus === "rejected") {
        toast.error(result.payload || "Google sign-in failed. Please try again.");
      }
    };
  ...
  }, [googleClientId, showForgot, googleLogin]);
          ...
          <AnimatePresence mode="wait">
            ...
            {showForgot ? (
              ...
              >
                ...
                {/* Google SSO — real Google Identity Services button */}
                {googleClientId ? (
                  <div className="flex w-full justify-center" ref={googleBtnRef} />
                ) : (
                  <button
                    type="button"
                    disabled
                    title="Set VITE_GOOGLE_CLIENT_ID in the frontend .env to enable this."
                    className="flex h-11 w-full items-center justify-center gap-2.5 rounded-button border border-warm-200 bg-warm-50 text-sm font-medium text-text-disabled cursor-not-allowed"
                  >
